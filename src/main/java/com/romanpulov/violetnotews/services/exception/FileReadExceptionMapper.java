...
public class FileReadExceptionMapper implements ExceptionMapper<FileReadException> {
    ...
    public Response toResponse(FileReadException e) {
        return Response.status(Response.Status.NOT_FOUND).entity(new RestExceptionResponse(Response.Status.NOT_FOUND.getStatusCode(), e.getMessage())).type(MediaType.APPLICATION_JSON).build();
    }
}
